...
// Scan for .md and .html files sorted alphabetically
const mdFiles = readdirSync(__dirname)
  .filter(f => f.endsWith('.md') || f.endsWith('.html'))
  .filter(f => f !== 'index.html' && f.toLowerCase() !== 'readme.md')
  .sort((a, b) => {
    const statA = statSync(join(__dirname, a));
    const statB = statSync(join(__dirname, b));
...
const documents = mdFiles.map(filename => {
  ...
  const mtime = statSync(join(__dirname, filename)).mtimeMs;
  return { filename, content, mtime };
});
...
const jsLines = [
  ...
  '  list.innerHTML = \'\';',
  '  sortedIndices.forEach(function(docIndex) {',
  '    const doc = DOCUMENTS[docIndex];',
  '    const btn = document.createElement(\'button\');',
  "    btn.className = 'doc-item';",
  '    if (docIndex === currentIndex) btn.classList.add(\'active\');',
  "    btn.textContent = doc.filename.replace(/\\.md$/, '').replace(/\\.html$/, '');",
...
const cssLines = [
  ...
  '}',
  '.sort-btn:hover { color: #1a1a1a; border-color: #d4d4d4; }',
  ...
  '.content code { font-family: \'JetBrains Mono\', monospace; font-size: 12px; background: #f0f0f0; padding: 3px 6px; border-radius: 4px; border: 1px solid #e0e0e0; color: #d6336c; }',
...
const htmlParts = [
  ...
  '  <title>Screens/Flow Inventory</title>',
  '  <style>',
  embeddedCss,
  '  </style>',
  '</head>',
  '<body>',
  '  <div class="topbar">',
  '    <div class="topbar-title">Screens/Flow Inventory</div>',
